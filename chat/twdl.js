...
bot.on(["/start"], (msg) => msg.reply.text(`Selamat datang di bot Twitter Downloader saya!\n\nPastekan link twittermu disini.`));
...
const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss');
 ...
 if (isUrl(msg.text)) {
if (!msg.text.includes("twitter.com")) return  msg.reply.text('Maaf link youtube tidak terdeteksi.');
  try {
  let hasil = await fetchJson(`https://api-xcoders.site/api/download/twitter?url=${findUrl(msg.text)[0]}&apikey=megalodon`);
  msg.reply.text('Sedang diproses');

  bot.sendChatAction(msg.chat.id, 'upload_video');
   let buff = await getBuffer(hasil.result.url);
  bot.sendChatAction(msg.chat.id, 'upload_video');
  let caption = `
Twitter Downloader

Caption : ${hasil.result.caption}
Username : ${hasil.result.username}
Name : ${hasil.result.name}
Verified : ${hasil.result.verified}
Create at : ${hasil.result.created_ad}
Like : ${hasil.result.like_count}
Retweet : ${hasil.result.retweet_count}
Reply : ${hasil.result.reply_count}
Size : ${await getFileSize(hasil.result.url)}
